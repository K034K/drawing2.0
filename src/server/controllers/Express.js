...
export default class Express {
    ...
    listen(type, ClassRoute) {
        if (type == "get") {
            console.log("listen", type, ClassRoute.getRoute);
            for (const route of ClassRoute.getRoute) {
        ...
        if (type == "post") {
            console.log("listen", type, ClassRoute.postRoute);
            for (const route of ClassRoute.postRoute) {
                this.app[type](route, (req, res, next) => {
                    console.log(req.url);
                    try {
                        new ClassRoute(req, res, next).onRequest({ db });
